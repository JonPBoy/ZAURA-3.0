...
// =====================================================================
// DAILY COSMIC READING — personal day number + today's moon + day ruler
// =====================================================================
export function computeDailyReading(profile, now = new Date()) {
  const [, bm, bd] = profile.birthDate.split('-').map(Number);
  const cy = now.getFullYear(), cm = now.getMonth() + 1, cd = now.getDate();
  const personalYear = reduceNum(reduceNum(bm) + reduceNum(bd) + reduceNum(digitSum(cy)), false);
  const personalMonth = reduceNum(personalYear + cm, false);
  const personalDay = reduceNum(personalMonth + cd, false);

  const THEMES = {
    1: ['Initiate', 'Plant a seed today. Start the thing you have been circling \u2014 the cosmos favors first moves.'],
    2: ['Attune', 'A day for listening, patience and partnership. Let things come to you; force nothing.'],
    3: ['Express', 'Your creative channel is open wide. Speak, write, play \u2014 joy is productive today.'],
    4: ['Build', 'Roll up your sleeves. Order, detail and steady effort turn intentions into foundations.'],
    5: ['Shift', 'Expect the unexpected and ride it. Change is the doorway \u2014 say yes to the detour.'],
    6: ['Tend', 'Home, love and responsibility call. Small acts of care carry unusual weight today.'],
    7: ['Retreat', 'A day for depth over noise. Study, meditate, walk alone \u2014 answers arrive in silence.'],
    8: ['Claim', 'Power day. Handle money, make the ask, take the meeting \u2014 the material world responds.'],
    9: ['Release', 'Complete and let go. Clear what is finished so the next cycle can find you empty-handed.'],
  };

  const J = jdn(cy, cm, cd);
  const age = mod(J - 2451550.1, 29.530588853);
  const PH = [
    [1.84, 'New Moon', '\uD83C\uDF11', 'Set intentions in the dark \u2014 what you whisper now grows all month.'],
    [5.53, 'Waxing Crescent', '\uD83C\uDF12', 'Feed your intentions with one small action. Momentum is fragile and precious.'],
    [9.22, 'First Quarter', '\uD83C\uDF13', 'Resistance appears on schedule. Decide, act, adjust \u2014 do not stall.'],
    [12.91, 'Waxing Gibbous', '\uD83C\uDF14', 'Refine and trust the process. You are closer than you feel.'],
    [16.61, 'Full Moon', '\uD83C\uDF15', 'Maximum light: emotions and truths surface. Celebrate, release, forgive.'],
    [20.30, 'Waning Gibbous', '\uD83C\uDF16', 'Share what you have learned. Gratitude turns harvest into wisdom.'],
    [23.99, 'Last Quarter', '\uD83C\uDF17', 'Prune what no longer serves. Endings now are mercy, not loss.'],
    [27.68, 'Waning Crescent', '\uD83C\uDF18', 'Rest is sacred work. Empty the cup so it can be filled again.'],
    [30, 'New Moon', '\uD83C\uDF11', 'Set intentions in the dark \u2014 what you whisper now grows all month.'],
  ];
  const phase = PH.find(([lim]) => age < lim);

  const DAY_RULERS = [
    ['Sun', 'vitality, visibility and heart \u2014 step into the light'],
    ['Moon', 'feeling, home and intuition \u2014 honor your tides'],
    ['Mars', 'courage, drive and decisive action \u2014 spend your fire well'],
    ['Mercury', 'words, trade and connection \u2014 send the message'],
    ['Jupiter', 'expansion, luck and vision \u2014 think bigger'],
    ['Venus', 'love, beauty and pleasure \u2014 make something lovely'],
    ['Saturn', 'discipline, boundaries and legacy \u2014 do the real work'],
  ];
  const ruler = DAY_RULERS[now.getDay()];
  const t = THEMES[personalDay];
...
// =====================================================================
// WEEKLY FORECAST — 7-day outlook with best days for love, work, rest
// =====================================================================
export function computeWeeklyForecast(profile, start = new Date()) {
  const LOVE = { 1: 55, 2: 90, 3: 78, 4: 55, 5: 68, 6: 92, 7: 50, 8: 60, 9: 74 };
  const WORK = { 1: 90, 2: 62, 3: 70, 4: 88, 5: 72, 6: 65, 7: 58, 8: 94, 9: 55 };
  const REST = { 1: 45, 2: 70, 3: 60, 4: 52, 5: 55, 6: 68, 7: 94, 8: 42, 9: 82 };
  const clamp = (n) => Math.max(20, Math.min(99, Math.round(n)));
  const days = [];
  for (let i = 0; i < 7; i++) {
    const d = new Date(start);
    d.setDate(d.getDate() + i);
    const daily = computeDailyReading(profile, d);
    const pd = daily.personalDay;
    const dow = d.getDay();
    let love = LOVE[pd], work = WORK[pd], rest = REST[pd];
    if (dow === 5) love += 8;   // Venus day
    if (dow === 0) love += 4;   // Sun day warmth
    if (dow === 1) rest += 6;   // Moon day
    if (dow === 6) work += 5;   // Saturn day
    if (dow === 2) work += 4;   // Mars day
    if (daily.moonPhase === 'Full Moon') { love += 6; rest -= 4; }
    if (daily.moonPhase.startsWith('Waning') || daily.moonPhase === 'Last Quarter') rest += 6;
    if (daily.moonPhase.startsWith('Waxing') || daily.moonPhase === 'First Quarter') work += 4;
    days.push({
      ...daily,
      weekday: d.toLocaleDateString(undefined, { weekday: 'short' }),
      dayNum: d.getDate(),
      isToday: i === 0,
      love: clamp(love), work: clamp(work), rest: clamp(rest),
    });
  }
  const bestOf = (k) => days.reduce((b, x) => (x[k] > b[k] ? x : b), days[0]);
  return { days, best: { love: bestOf('love'), work: bestOf('work'), rest: bestOf('rest') } };
}
...
// =====================================================================
// MOON CALENDAR — full-month moon phases + personal power days
// =====================================================================
const MOON_PHASES = [
  { name: 'New Moon',        glyph: '\uD83C\uDF11', text: 'Set intentions in the dark \u2014 what you whisper now grows all month.' },
  { name: 'Waxing Crescent', glyph: '\uD83C\uDF12', text: 'Feed intentions with one small action. Momentum is fragile and precious.' },
  { name: 'First Quarter',   glyph: '\uD83C\uDF13', text: 'Resistance appears on schedule. Decide, act, adjust \u2014 do not stall.' },
  { name: 'Waxing Gibbous',  glyph: '\uD83C\uDF14', text: 'Refine and trust the process. You are closer than you feel.' },
  { name: 'Full Moon',       glyph: '\uD83C\uDF15', text: 'Maximum light: emotions and truths surface. Celebrate, release, forgive.' },
  { name: 'Waning Gibbous',  glyph: '\uD83C\uDF16', text: 'Share what you have learned. Gratitude turns harvest into wisdom.' },
  { name: 'Last Quarter',    glyph: '\uD83C\uDF17', text: 'Prune what no longer serves. Endings now are mercy, not loss.' },
  { name: 'Waning Crescent', glyph: '\uD83C\uDF18', text: 'Rest is sacred work. Empty the cup so it can be filled again.' },
];
const PHASE_EDGES = [1.84, 5.53, 9.22, 12.91, 16.61, 20.30, 23.99, 27.68];
...
export function getMoonPhase(date = new Date()) {
  ...
  const age = mod(J - 2451550.1, 29.530588853);
  let idx = 7;
  for (let i = 0; i < PHASE_EDGES.length; i++) { if (age < PHASE_EDGES[i]) { idx = i; break; } }
  // Illumination approx from age: 0 at new, 1 at full
  const illumination = Math.max(0, Math.min(1, (1 - Math.cos((age / 29.530588853) * 2 * Math.PI)) / 2));
  const isKey = MOON_PHASES[idx].name === 'New Moon' || MOON_PHASES[idx].name === 'Full Moon';
  return { index: idx, ...MOON_PHASES[idx], age, illumination, isKeyPhase: isKey };
}

// Power day = personal day equals user's life-path OR the day is a New/Full moon
export function isPowerDay(profile, date) {
  const daily = computeDailyReading(profile, date);
  const moon = getMoonPhase(date);
  // Compute life-path (reduced) — mirrors numerology block in computeAllModalities
  const [y, m, d] = profile.birthDate.split('-').map(Number);
  const lifePath = reduceNum(digitSum(y) + digitSum(m) + digitSum(d));
  const lifePathSingle = toSingle(lifePath);
  const powerDaySet = new Set([1, 8, lifePathSingle]);
  const byNumber = powerDaySet.has(daily.personalDay);
  const byMoon = moon.isKeyPhase;
  return { isPower: byNumber || byMoon, byNumber, byMoon, lifePath: lifePathSingle, personalDay: daily.personalDay, moonPhaseName: moon.name };
}

// Build a full-month 6x7 grid (42 cells) with padding to the previous/next month
export function getMonthMoonCalendar(profile, year, month /* 1-12 */) {
  ...
  for (let i = startDow - 1; i >= 0; i--) {
    ...
    cells.push(buildCell(profile, d, false, todayKey));
  }
  for (let d = 1; d <= daysInMonth; d++) {
    cells.push(buildCell(profile, new Date(year, month - 1, d), true, todayKey));
  }
  // pad to 42
  let nextD = 1;
  while (cells.length < 42) {
    cells.push(buildCell(profile, new Date(year, month, nextD++), false, todayKey));
  }
  // month meta
  const label = first.toLocaleDateString(undefined, { month: 'long', year: 'numeric' });
  const powerCount = cells.filter((c) => c.inMonth && c.isPower).length;
  const keyMoons = cells.filter((c) => c.inMonth && c.moon.isKeyPhase).map((c) => ({ day: c.day, name: c.moon.name, glyph: c.moon.glyph }));
  return { year, month, label, cells, powerCount, keyMoons };
}

function buildCell(profile, dateObj, inMonth, todayKey) {
  const y = dateObj.getFullYear(), m = dateObj.getMonth() + 1, d = dateObj.getDate();
  const daily = computeDailyReading(profile, dateObj);
  const moon = getMoonPhase(dateObj);
  const pd = isPowerDay(profile, dateObj);
  const key = `${y}-${m}-${d}`;
  return {
    key,
    day: d,
    ...
    themeName: daily.themeName,
    themeText: daily.themeText,
    isPower: pd.isPower,
    powerReason: pd.byNumber && pd.byMoon ? 'both' : pd.byNumber ? 'number' : pd.byMoon ? 'moon' : null,
  };
}
